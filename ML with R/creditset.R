df = read.csv('creditset.csv', nrows=100)
head(df)

df[c("clientid", "LTI")] =NULL
library(caTools)
...
#visualization
#library(ggplot2)
#ggplot()+ geom_point(aes(x=train_set$loan, y = train_set$default10yr),data=train_set, color="blue")+
 # geom_line(aes(x=train_set$loan, y = predict(classifier, newdata=train_set, type="response")), data= train_set, color="green")
